...
colData(se)$log_pTau <- log(colData(se)$hippocampus_proper_aperio_quants + 1)
cg_ok = (checkLevels(assay(se), colData(se)$sex) &
    checkLevels(assay(se), colData(se)$sample_plate))
sample_ok <- !is.na(colData(se)$log_pTau) & !is.na(colData(se)$age)
fml <- as.formula("~age + sex + sample_plate + log_pTau")

res <- DML(se[cg_ok,sample_ok],fm=fml,BPPARAM=BiocParallel::MulticoreParam(40))
smry <- summaryExtractTest(res)
smry$FDR_log_pTau <- p.adjust(smry$Pval_log_pTau,method="fdr")
saveRDS(smry,file="~/PART_AD/PWG/PWG_PART_pTau_EWAS.rds")
```
...
#cpg_gene_corr_all %>% filter(FDR < .0001, abs(Cor) >= .2) %>%
 #pull(Gene) %>% unique()
```
